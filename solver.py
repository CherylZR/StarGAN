...
class Solver(object):
    ...
    def __init__(self, celebA_loader, rafd_loader, config):  # exists undone
        # Data loader
    ...
    def make_celeb_labels(self, real_c):
        ...
        # single attribute transfer Blonde, Eglasses, Male, Smile, Young
        ...
        #        fixed_c_list.append(self.to_var(fixed_c, volatile=True))
        # multi-attribute transfer (B+E, B+M, M+S, B+S+Y)
        if self.dataset == 'CelebA':
            for i in range(4):
                fixed_c = real_c.clone()
                for c in fixed_c:
        ...
        for e in range(start, self.num_epochs):
            for i, (real_x, real_label) in enumerate(self.data_loader):
                ...
                if (i+1) % self.log_step == 0:
                    ...
                    if self.dataset == 'CelebA':
                        print('Classification Acc (Blond/Eyeglasses/Male/Smiling/Young): ')
                    else:
